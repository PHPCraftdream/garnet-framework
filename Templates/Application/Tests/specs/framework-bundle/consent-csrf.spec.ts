...
test.describe('Consent-gated CSRF', () => {
    test('cold load: no cookies, no CSRF marker before any interaction', async ({ browser }) => {
        const context = await newScopedContext(browser, { baseURL: BASE });
        const page = await context.newPage();

        // Navigating straight to the auth route without ticking consent must
        // not mint anything — the widget itself is what we assert against
        // below, so this only checks the pre-interaction state on load.
    ...
    test('auth form: cookies appear after checking the consent box', async ({ browser }) => {
        ...
        const [startSessResp] = await Promise.all([
            page.waitForResponse(
                r => r.request().method() === 'POST' && r.url().includes(AUTH_PATH),
                { timeout: 15000 },
            ),
