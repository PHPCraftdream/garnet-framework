 ...
 *
 *   1. Unknown email → 403 "registrations disabled" message
...
// NOT the .test registration-gate carve-out.
...
function setRegistrationsEnabled(enabled: boolean) {
    ...
    if (/^registrations_enabled\s*=\s*\d+/m.test(text)) {
        text = text.replace(
...
test.describe('Registration gate — registrations_enabled toggle', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        // The registration-gate check runs inside a CSRF-protected POST, and
        // CSRF is not minted until the user ticks the PD consent (which fires
        // `start-session`). Drive the consent flow before any assertions run.
