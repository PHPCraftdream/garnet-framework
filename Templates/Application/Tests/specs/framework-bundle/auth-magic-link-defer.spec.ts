 ...
 *
 * Regression test for the bug where opening `/#token=<CODE>` from a
 * fresh browser (no session cookie → server renders phase=INPUT_EMAIL)
 * caused the #token hash to be consumed and destroyed before the phase
 * advanced to INPUT_CODE, silently breaking cross-device magic-link
 * logins. The hash-capture logic lives entirely in
 * `Bundle/Front/auth/Auth2.tsx` and is independent of any invite flow.
 *
 * Two flows:
 *
...
test.describe('Deferred hash capture — magic-link survives phase transition', () => {
    ...
    test('cross-device: deferred code auto-submits after email triggers INPUT_CODE phase', async ({ browser }) => {
        ...
        // The fix captures the hash code into React state. After the
        ...
        page.on('response', (r) => {
            if (r.request().method() !== 'POST' || !r.url().includes('/')) return;
            const raw = r.request().postData() ?? '';
            if (raw.includes('"action":"start-session"')) return;
            allPosts.push(r);
        });

        await page.goto(`${AUTH_PATH}#token=${code}`);
