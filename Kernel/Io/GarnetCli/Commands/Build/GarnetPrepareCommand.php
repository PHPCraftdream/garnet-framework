...
class GarnetPrepareCommand {
    ...
    /**
     * Пишет `Foreground/VersionGen.php` — версия/коммит приложения на момент
     * сборки, plain-PHP `return [...]` без namespace/класса (у каждого
     * приложения свой namespace, автозагрузка не нужна — читатель просто
     * делает `require`). Тот же *Gen.php-конвейер, что и у остальных
     * генерируемых файлов: не в git, пересобирается каждым `prepare`,
     * довозится деплоем как billed asset (см. PublicPathRebrander::genFiles()).
     *
     * Мягкий отказ: без git или без Foreground/ — просто не пишет файл,
     * сборка не падает.
     */
    private static function generateVersionGen(object $app): void {
        $appDir = rtrim((string)$app->appDir, '/\\') . DIRECTORY_SEPARATOR;
        $foregroundDir = $appDir . 'Foreground';

        if (!is_dir($foregroundDir)) {
            return;
        }

        $describe = self::gitOutput($appDir, ['describe', '--tags', '--always', '--dirty', '--abbrev=7']);
        $shortSha = self::gitOutput($appDir, ['rev-parse', '--short=7', 'HEAD']);

        if ($describe === null && $shortSha === null) {
            return;
        }

        $appVersion = $describe ?? $shortSha;
        $appCommit = $shortSha ?? $describe;
        $builtAt = date('c');
        ...
        $content = "<?php declare(strict_types=1);\n\n"
            . "// Generated by `php garnet prepare` — not tracked in git, do not edit by hand.\n"
            . "return {$export};\n";

        file_put_contents($foregroundDir . DIRECTORY_SEPARATOR . 'VersionGen.php', $content);
    }

    /** Runs a git subcommand with cwd=$appDir; null on any failure (no git, not a repo, …). */
    private static function gitOutput(string $appDir, array $args): ?string {
        $cmd = 'git ' . implode(' ', array_map('escapeshellarg', $args)) . ' 2>' . (DIRECTORY_SEPARATOR === '\\' ? 'NUL' : '/dev/null');
        $prevDir = getcwd();
