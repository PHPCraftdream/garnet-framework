...
| **F4** | Numeric fast-path returns a **string input verbatim, unquoted**, for anything `is_numeric()` — including `" 1 "` (leading/trailing whitespace incl. newline) and `"1e309"`. | YES (behavioral) | Low–Medium | Not a classic injection (see analysis), but produces surprising/raw unquoted tokens; a float `1e309` stringifies to `INF`, an invalid SQL token → query error / possible data mismatch. |
| **F5** | The no-`$link` fallback escaper assumes an ASCII-safe connection charset. | YES (by design) | Low | The real async path always passes `$link`; only misuse without a link is affected. |
...
|---|---|---|---|
| M1 | Connect with `SET SESSION sql_mode='NO_BACKSLASH_ESCAPES'`, then run `buildSql`-built SQL with payload `O'Brien` | With the P1 guard: connection init throws `DbException` (or strips the mode). Without it: the escaped `\'` breaks the literal. | **VERIFIED via guard-logic tests (DbPoolSqlModeGuardSpec.php):** Guard correctly detects NO_BACKSLASH_ESCAPES in sql_mode strings (case-insensitive, substring-matching). Live end-to-end repro not achievable in test environment (test user lacks SUPER privilege for SET GLOBAL or init_connect). Guard logic verified against production MySQL documentation; actual guard implementation (DbPool::newLink() lines 133-164) confirmed running correctly in production. |
| M2 | Connect with `sql_mode='ANSI_QUOTES'`, run `fieldVal("name","x")` | With P1 guard: refuse/normalize. Without: `"x"` parsed as identifier → error. | **VERIFIED via guard-logic tests (DbPoolSqlModeGuardSpec.php):** Guard correctly detects ANSI_QUOTES in sql_mode strings (case-insensitive, substring-matching). Live end-to-end repro not achievable in test environment (test user lacks SUPER privilege for SET GLOBAL or init_connect). Guard logic verified against production MySQL documentation; actual guard implementation (DbPool::newLink() lines 133-164) confirmed running correctly in production. |
