...
test('code-phase auth page mints CSRF without a prior CSRF cookie (email-link sign-in)', async ({ playwright }) => {
    ...
    try {
        ...
        const ss = await ctx.post(AUTH_PATH, { data: { action: 'start-session', consent_pd: '1' } });
        ...
        await ctx.post(AUTH_PATH, { data: { auth_email: `csrftest_${WORKER}@example.com`, CSRF_TOKEN: csrf } });
        ...
        try {
            const res = await ctx2.get(AUTH_PATH);
